...
const OV  = {position:"fixed",inset:0,background:"rgba(15,23,42,0.65)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:300,backdropFilter:"blur(3px)"};
    ...
    <div style={OV}>
      <div style={{background:"#fff",border:"1px solid #E2E8F0",borderRadius:14,boxShadow:"0 20px 60px rgba(0,0,0,0.2)",width:isMobile?"95vw":"580px",maxHeight:"90vh",display:"flex",flexDirection:"column"}}>
        <div style={{padding:"18px 24px",borderBottom:"1px solid #E2E8F0",flexShrink:0}}>
          <h2 style={{color:"#1E293B",fontSize:17,fontWeight:700,margin:0}}>⚠️ Verificar entrada antes de confirmar</h2>
          <div style={{color:"#94A3B8",fontSize:12,marginTop:4}}>Revisa los detalles. Esta acción actualizará el inventario.</div>
        </div>
        <div style={{flex:1,overflowY:"auto",padding:24}}>
          {/* Info general */}
          <div style={{background:C.panel,borderRadius:10,padding:14,marginBottom:16,border:"1px solid #E2E8F0"}}>
          ...
          <div style={{border:"1px solid #E2E8F0",borderRadius:8,overflow:"hidden"}}>
            <div style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr",gap:6,padding:"8px 12px",background:C.panel}}>
              {["PRODUCTO","EXIST. ANT.","ENTRADA","EXIST. ACT."].map(h=>(
                <span key={h} style={{color:C.textSm,fontSize:11,fontWeight:600,textAlign:h==="PRODUCTO"?"left":"center"}}>{h}</span>
              ))}
            </div>
            {data.lineas.map((l,i)=>(
              <div key={i} style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr",gap:6,padding:"10px 12px",borderTop:"1px solid #E2E8F0",background:"#fff"}}>
                ...
                <div style={{textAlign:"center",color:C.textMd,fontWeight:600,fontSize:14}}>{l.existAnt}</div>
                <div style={{textAlign:"center",color:C.blue,fontWeight:700,fontSize:14}}>+{l.cantidad}</div>
                <div style={{textAlign:"center",color:C.green,fontWeight:700,fontSize:14}}>{l.existNueva}</div>
              </div>
            ))}
            {modoAdmin&&(
              <div style={{display:"flex",justifyContent:"flex-end",padding:"10px 12px",background:C.blueBg,borderTop:"1px solid #BFDBFE"}}>
                <span style={{color:C.blue,fontWeight:700,fontSize:14}}>Total: {fmt(data.total)}</span>
              </div>
            )}
          </div>
        </div>
...
// ─── TICKET MODAL ─────────────────────────────────────────────────────────────
function TicketModal({ ticket, modoAdmin, onClose, isMobile }) {
  return (
    <div style={OV}>
      <div style={{background:"#fff",border:"1px solid #E2E8F0",borderRadius:14,boxShadow:"0 20px 60px rgba(0,0,0,0.2)",width:isMobile?"95vw":"440px",maxHeight:"90vh",overflowY:"auto",padding:24}}>
        <div style={{fontFamily:"'Courier New',monospace"}}>
          <div style={{textAlign:"center",borderBottom:"1px dashed #E2E8F0",paddingBottom:12,marginBottom:12}}>
            ...
            <div style={{fontSize:11,color:C.textMd}}>{ticket.sucursal}</div>
            <div style={{fontSize:11,color:C.textMd}}>{ticket.fechaReal}</div>
          </div>
          ...
          <div style={{borderTop:"1px dashed #E2E8F0",paddingTop:10,marginBottom:10}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr",gap:4,marginBottom:6,paddingBottom:6,borderBottom:"1px solid #E2E8F0"}}>
              {["PRODUCTO","ANT.","ENTRADA","ACT."].map(h=>(
                <span key={h} style={{color:C.textSm,fontSize:10,fontWeight:600,textAlign:h==="PRODUCTO"?"left":"center"}}>{h}</span>
              ))}
            </div>
          ...
          {modoAdmin&&(
            <div style={{borderTop:"1px dashed #E2E8F0",paddingTop:10,marginBottom:12}}>
              <div style={{display:"flex",justifyContent:"space-between"}}>
                ...
                <span style={{color:C.blue,fontSize:14,fontWeight:700}}>{fmt(ticket.total)}</span>
              </div>
            </div>
          )}
  ...
  const confirmarEntrada = async () => {
    ...
    try {
      const fechaVence = tipoPago==="credito"
        ? new Date(Date.now()+parseInt(diasCredito)*24*60*60*1000).toISOString() : null;
      ...
      for(const l of previewData.lineas) {
        const stockActual = l.existAnt;
        const costoActual = parseFloat(l.prod?.costo||0);
        const nuevoCosto  = modoAdmin && (stockActual+l.cantidad)>0
          ? ((stockActual*costoActual)+(l.cantidad*l.costo))/(stockActual+l.cantidad) : costoActual;
                ...
                <div style={{background:C.panel,borderRadius:10,padding:16,marginBottom:16,border:`1px solid ${C.border}`}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                    <div>
                      ...
                      <div style={{color:C.textSm,fontSize:12}}>Por: <strong>{entradaSel.usuario}</strong> · {entradaSel.sucursal}</div>
                    </div>
                    <div style={{textAlign:"right"}}>
